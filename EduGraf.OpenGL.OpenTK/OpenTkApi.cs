...
internal class OpenTkApi : GlApi
{
    ...
    private static DrawElementsType GetElementsType<T>()
    {
    ...
    protected override void Enable(GlCap cap)
    {
    ...
    protected override void DrawBuffers(GlFramebufferAttachment[] attachments)
    {
        var count = attachments.Length;
        var buffers = new DrawBuffersEnum[count];
        for (int i = 0; i < count; i++) buffers[i] = (DrawBuffersEnum)attachments[i];
    ...
    protected override void DeleteProgram(uint handle) { GL.DeleteProgram(handle); }
    ...
    protected override void Uniform2(uint location, float[] values) { GL.Uniform2((int)location, values.Length, values); }
    ...
    protected override void UniformMatrix2(uint location, bool transpose, float[] elements)
    {
        if (elements.Length != 4) throw new ArgumentException(nameof(elements));
    ...
    protected override void UniformMatrix3(uint location, bool transpose, float[] elements)
    {
        if (elements.Length != 9) throw new ArgumentException(nameof(elements));
    ...
    private static PixelInternalFormat GetInternal(GlPixelFormat format) => (PixelInternalFormat)format;
}